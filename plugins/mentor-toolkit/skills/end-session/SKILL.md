...
2. **빈 항목만 질문**: fetch 후 비어있는 섹션만 1개 메시지에 묶어 묻는다. 이미 있는 내용은 다시 묻지 않는다.
...
🪜 다음 권장 액션
- *"OO님 자소서 검토해줘"* → between-sessions (회차 사이 결과물 검토·미완 과제 처리)
- *"OO님 {N+1}차 시작"* → start-session (다음 회차 브리핑·페이지 자동 생성)
```
