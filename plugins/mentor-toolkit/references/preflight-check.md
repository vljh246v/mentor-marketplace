...
| `company-recommender` | 멘티 분석 완료 권장. Scrapling MCP가 있으면 라이브 공고 수집에 우선 사용하고, 없으면 WebFetch/WebSearch 또는 사용자 제공 JD로 fallback. Scrapling 미사용 결과에는 수집 방식과 검증 신뢰도 제한을 표시 |
| `resume-reviewer` / `interview-prep` | 멘티 분석 완료 권장 (없으면 일반 답변) |
