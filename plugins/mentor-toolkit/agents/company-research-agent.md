...
분야·연차·스택을 조합해 1~3개 쿼리 생성. **분야별 쿼리 패턴**:

| 분야 | 쿼리 예시 |
|------|---------|
| 백엔드 / 프론트엔드 / 풀스택 / 모바일 / 데이터·AI / DevOps | `백엔드 Spring 3년차`, `Java/Kotlin 백엔드 3~5년`, `핀테크 백엔드` |
| UI/UX·시각·제품 디자인 | `UX 디자이너 3년차`, `프로덕트 디자이너 시니어`, `UI 디자인 핀테크` |
| 서비스 기획 / PM / PO / 마케팅 / 그로스 | `프로덕트 매니저 3년차`, `서비스 기획 시니어`, `그로스 마케터` |
...
- 연봉이 "회사 비공개"면 그렇게 명시
- 회사명·포지션은 원문 그대로
- 멘토 시점 의견은 넣지 않음 (점수 매기는 건 호출자 스킬의 역할)
